...
public class ExternalEditorServiceTests : IDisposable
{
    ...
    [InlineData("\"C:\\Program Files\\Editor\\edit.exe\" -n --goto 12", "C:\\Program Files\\Editor\\edit.exe", new[] { "-n", "--goto", "12" })]
    [InlineData("notepad.exe", "notepad.exe", new string[0])]
    [InlineData("   ", "", new string[0])]
    public void ParseCommandLine_ReturnsExpectedExecutableAndArguments(string input, string expectedExe, string[] expectedArgs)
    {
        var (exe, args) = ExternalEditorService.ParseCommandLine(input);
        exe.Should().Be(expectedExe);
        args.Should().BeEquivalentTo(expectedArgs);
    }

    [Fact]
    public void TryOpenScript_ReturnsFalse_WhenLauncherThrowsException()
    {
        var launcher = new FakeProcessLauncher
        {
            ThrowOnLaunch = new InvalidOperationException("Process failed to start")
        };
        var service = new ExternalEditorService(launcher);
        var script = CreateTempScript();

        var ok = service.TryOpenScript(script, "bad_editor.exe", out var warning);

        ok.Should().BeFalse();
        warning.Should().Contain("bad_editor.exe");
    }
}
