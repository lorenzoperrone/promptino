...
public partial class MainWindow : Window
{
    ...
    private async void OnMainWindowLoaded(object? sender, RoutedEventArgs e)
    {
        ...
        ClickThroughCheckBox.AddHandler(ToggleButton.IsCheckedChangedEvent, (_, _) => ClickThroughChanged());
        TargetScreenComboBox.SelectionChanged += TargetScreenChanged;
        PrompterFullscreenCheckBox.AddHandler(ToggleButton.IsCheckedChangedEvent, (_, _) => PrompterFullscreenChanged());
        ShowPresentationTimerCheckBox.AddHandler(ToggleButton.IsCheckedChangedEvent, (_, _) => ShowPresentationTimerChanged());
        TargetPresentationMinutesNumeric.ValueChanged += TargetPresentationMinutesChanged;
        ...
        ClickThroughCheckBox.IsChecked = _viewModel.ClickThroughEnabled;
        PrompterFullscreenCheckBox.IsChecked = _viewModel.PrompterFullscreen;
        ShowPresentationTimerCheckBox.IsChecked = _viewModel.ShowPresentationTimer;
        TargetPresentationMinutesNumeric.Value = _viewModel.TargetPresentationMinutes;
    ...
    private async void LoadScriptButton_OnClick(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        ...
        _prompterWindow.SetPresentationTimerConfig(_viewModel.ShowPresentationTimer, _viewModel.TargetPresentationMinutes);
        ApplyAlwaysOnTopPreference();
        ApplyScreenShareSafeModePreference();
        ApplyClickThroughToPrompter();

        if (_lastPrompterBounds is not null)
            _prompterWindow.RestoreBounds(_lastPrompterBounds.Value);

        ApplyTargetScreenToPrompter();
    ...
    {
        TargetScreenComboBox.Items.Clear();
        ...
        if (screens != null && screens.Count > 0)
        {
            ...
            {
                var scr = screens[i];
                var suffix = scr.IsPrimary ? Localizer.Get("LabelPrimarySuffix") : string.Empty;
                var name = Localizer.Get("LabelDisplay", i + 1, scr.Bounds.Width, scr.Bounds.Height) + suffix;
                TargetScreenComboBox.Items.Add(new ComboBoxItem { Content = name, Tag = i });
            }
            int idx = Math.Clamp(_viewModel.TargetScreenIndex, 0, screens.Count - 1);
            TargetScreenComboBox.SelectedIndex = idx;
        }
        else
        {
            TargetScreenComboBox.Items.Add(new ComboBoxItem { Content = Localizer.Get("LabelPrimaryDisplay"), Tag = 0 });
            TargetScreenComboBox.SelectedIndex = 0;
        }
    }

    private void ClickThroughChanged()
    {
        _viewModel.SetClickThroughEnabled(ClickThroughCheckBox.IsChecked == true);
        _ = _viewModel.SavePreferencesAsync();
        ApplyClickThroughToPrompter();
    }

    private void ApplyClickThroughToPrompter()
    {
        if (_prompterWindow != null && _prompterWindow.TryGetPlatformHandle()?.Handle is IntPtr handle && handle != IntPtr.Zero)
        {
            var service = OperatingSystem.IsWindows() ? (Promptino.Platform.IWindowClickThroughService)new Promptino.Platform.WindowClickThroughService() : new Promptino.Platform.NoOpWindowClickThroughService();
            service.SetClickThrough(handle, _viewModel.ClickThroughEnabled);
        }
    }

    private void TargetScreenChanged(object? sender, SelectionChangedEventArgs e)
    {
        if (TargetScreenComboBox.SelectedItem is ComboBoxItem item && item.Tag is int idx)
        {
            _viewModel.SetTargetScreenIndex(idx);
            _ = _viewModel.SavePreferencesAsync();
            ApplyTargetScreenToPrompter();
        }
    }

    private void PrompterFullscreenChanged()
    {
        _viewModel.SetPrompterFullscreen(PrompterFullscreenCheckBox.IsChecked == true);
        _ = _viewModel.SavePreferencesAsync();
        ApplyTargetScreenToPrompter();
    }

    private void ShowPresentationTimerChanged()
    {
        _viewModel.SetShowPresentationTimer(ShowPresentationTimerCheckBox.IsChecked == true);
        _ = _viewModel.SavePreferencesAsync();
        _prompterWindow?.SetPresentationTimerConfig(_viewModel.ShowPresentationTimer, _viewModel.TargetPresentationMinutes);
    }

    private void TargetPresentationMinutesChanged(object? sender, NumericUpDownValueChangedEventArgs e)
    {
        if (e.NewValue.HasValue)
        {
            _viewModel.SetTargetPresentationMinutes((int)e.NewValue.Value);
            _ = _viewModel.SavePreferencesAsync();
            _prompterWindow?.SetPresentationTimerConfig(_viewModel.ShowPresentationTimer, _viewModel.TargetPresentationMinutes);
        }
    }

    private void ApplyTargetScreenToPrompter()
    {
        if (_prompterWindow == null) return;

        var screens = Screens.All;
        if (screens != null && screens.Count > 0)
        {
            int idx = Math.Clamp(_viewModel.TargetScreenIndex, 0, screens.Count - 1);
            var targetScr = screens[idx];

            if (_viewModel.PrompterFullscreen)
            {
                _prompterWindow.WindowState = WindowState.Normal;
                _prompterWindow.Position = new PixelPoint(targetScr.Bounds.X, targetScr.Bounds.Y);
                _prompterWindow.WindowState = WindowState.FullScreen;
            }
            else
            {
                _prompterWindow.WindowState = WindowState.Normal;
                var x = targetScr.Bounds.X + (targetScr.Bounds.Width - (int)_prompterWindow.Width) / 2;
                var y = targetScr.Bounds.Y + (targetScr.Bounds.Height - (int)_prompterWindow.Height) / 2;
                _prompterWindow.Position = new PixelPoint(x, y);
            }
        }
    }
    ...
    private void ResetButton_OnClick(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        ...
        _prompterWindow?.ResetSpeechTimer();
