...
public sealed class MainWindowViewModel : INotifyPropertyChanged, IDisposable
{
    ...
    public bool ClickThroughEnabled { get => _clickThroughEnabled; private set => Set(ref _clickThroughEnabled, value); }
    private int _targetScreenIndex;
    public int TargetScreenIndex { get => _targetScreenIndex; private set => Set(ref _targetScreenIndex, value); }
    private bool _prompterFullscreen;
    public bool PrompterFullscreen { get => _prompterFullscreen; private set => Set(ref _prompterFullscreen, value); }
    private int _targetPresentationMinutes;
    public int TargetPresentationMinutes { get => _targetPresentationMinutes; private set => Set(ref _targetPresentationMinutes, value); }
    private bool _showPresentationTimer;
    public bool ShowPresentationTimer { get => _showPresentationTimer; private set => Set(ref _showPresentationTimer, value); }
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        ClickThroughEnabled = settings.EffectiveClickThroughEnabled;
        TargetScreenIndex = settings.EffectiveTargetScreenIndex;
        PrompterFullscreen = settings.EffectivePrompterFullscreen;
        TargetPresentationMinutes = settings.EffectiveTargetPresentationMinutes;
        ShowPresentationTimer = settings.EffectiveShowPresentationTimer;
    }
    ...
    public async Task SavePreferencesAsync(CancellationToken cancellationToken = default)
    {
        ...
        var saved = await _settingsStore.SaveAsync(new AppSettings(
            ...
            PrompterFullscreen: PrompterFullscreen,
            TargetPresentationMinutes: TargetPresentationMinutes,
            ShowPresentationTimer: ShowPresentationTimer), cancellationToken);
        if (saved)
        {
    ...
    public void SetClickThroughEnabled(bool value) => ClickThroughEnabled = value;
    public void SetTargetScreenIndex(int index) => TargetScreenIndex = Math.Max(0, index);
    public void SetPrompterFullscreen(bool value) => PrompterFullscreen = value;
    public void SetTargetPresentationMinutes(int minutes) => TargetPresentationMinutes = Math.Max(0, minutes);
    public void SetShowPresentationTimer(bool value) => ShowPresentationTimer = value;
