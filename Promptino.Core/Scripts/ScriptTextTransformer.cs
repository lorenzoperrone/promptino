...
public sealed partial class ScriptTextTransformer
{
    ...
    private static string ApplyLineFilters(string input, ScriptCleanupOptions options)
    {
        ...
        var inFence = false;
        var pendingFenceLines = new List<string>();
        ...
        for (var i = 0; i < lines.Length; i++)
        {
            ...
            // Fenced code blocks: skip entirely if closed properly
            if (trimmedStart.StartsWith("```", StringComparison.Ordinal))
            {
                if (!inFence)
                {
                    inFence = true;
                    pendingFenceLines.Clear();
                    pendingFenceLines.Add(line);
                }
                else
                {
                    inFence = false;
            ...
            if (options.RemoveMetadataRows && i == 0 && trimmed == "---" && HasClosingFrontMatter(lines, 1))
            {
        ...
        // If fenced code block was never closed, retain original lines to prevent script blanking
        if (inFence && pendingFenceLines.Count > 0)
        {
            output.AddRange(pendingFenceLines);
        }

        return string.Join('\n', output);
    }

    private static bool HasClosingFrontMatter(string[] lines, int startIndex, int maxLookahead = 40)
    {
        int limit = Math.Min(lines.Length, startIndex + maxLookahead);
